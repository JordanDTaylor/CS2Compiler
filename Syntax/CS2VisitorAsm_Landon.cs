...
namespace Syntax
{
    partial class CS2VisitorAsm : CS2BaseVisitor<object>
    {
        ...
        { // TODO value needs to be updated in the dictionary.
            ...
            ;
            */
            IParseTree left = context.GetChild(0);
            IParseTree right = context.GetChild(1);
            double value;
            UnaryOperator opp;
            ...
            if (left.GetType() == typeof(CS2Parser.Pre_unary_operatorContext)) // pre_unary_operator expression
            {
                opp = (UnaryOperator) Visit(context.pre_unary_operator()); 
                value = (double)Visit(right);
            }
            else // expression post_unary_operator
            {
                value = (double)Visit(left);
